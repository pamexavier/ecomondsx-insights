...
export function KeywordMarquee({
  ...
  return (
    <div
      className="relative w-screen overflow-hidden border-y border-border py-3"
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, oklch(0.17 0.008 152), transparent)",
        }}
      />
        ...
        {items.map((kw, i) => {
          ...
          return (
            <span
              key={`${kw}-${i}`}
              onClick={() => onSelect?.(kw)}
              className={`text-mono shrink-0 cursor-pointer border px-4 py-2 text-[11px] uppercase tracking-[0.15em] transition-colors
              ${
                isActive
